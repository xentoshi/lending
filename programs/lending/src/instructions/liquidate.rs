...
use crate::{constants::{MAX_AGE, SOL_USD_FEED_ID, USDC_USD_FEED_ID}, error::LendingError, state::{Bank, User}};
...
// if so, calculate liquidation amount
// make a CPI transfer from the user's token account to the bank's token account
// update the user and bank states
...
pub fn process_liquidate(ctx: Context<Liquidate>, amount: u64) -> Result<()> {
    ...
    let sol_price = price_update.get_price_no_older_than(&Clock::get().unwrap(), MAX_AGE, &sol_feed_id)?;
    let usdc_price = price_update.get_price_no_older_than(&Clock::get().unwrap(), MAX_AGE, &usdc_feed_id)?;
